...
use serde_json;
use test;
use sophon_wasm::{self, elements, builder};
use sophon_wasm::interpreter::{
    RuntimeValue,
...
fn try_deserialize(base_dir: &str, module_path: &str) -> Result<elements::Module, elements::Error> {
    ...
    sophon_wasm::deserialize_file(&wasm_path)
}

fn try_load(base_dir: &str, module_path: &str) -> Result<(), DummyInterpreterError> {
    let module = try_deserialize(base_dir, module_path).map_err(|e| sophon_wasm::interpreter::Error::Program(format!("{:?}", e)))?;
    let program = DefaultProgramInstance::new().expect("Failed creating program");
    program.add_module("try_load", module, None).map(|_| ())
}

fn runtime_value(test_val: &test::RuntimeValue) -> sophon_wasm::RuntimeValue {
    match test_val.value_type.as_ref() {
        "i32" => {
            let unsigned: u32 = test_val.value.parse().expect("Literal parse error");
            sophon_wasm::RuntimeValue::I32(unsigned as i32)
        },
        "i64" => {
            let unsigned: u64 = test_val.value.parse().expect("Literal parse error");
            sophon_wasm::RuntimeValue::I64(unsigned as i64)
        },
        "f32" => {
            let unsigned: u32 = test_val.value.parse().expect("Literal parse error");
            sophon_wasm::RuntimeValue::decode_f32(unsigned)
        },
        "f64" => {
            let unsigned: u64 = test_val.value.parse().expect("Literal parse error");
            sophon_wasm::RuntimeValue::decode_f64(unsigned)
        },
        _ => panic!("Unknwon runtime value type"),
    }
}

fn runtime_values(test_vals: &[test::RuntimeValue]) -> Vec<sophon_wasm::RuntimeValue> {
    test_vals.iter().map(runtime_value).collect::<Vec<sophon_wasm::RuntimeValue>>()
}

fn run_action(program: &DefaultProgramInstance, action: &test::Action) 
    -> Result<Option<sophon_wasm::RuntimeValue>, DummyInterpreterError> 
{
    match *action {
    ...
    for command in &spec.commands {
        ...
        match command {
            ...
            &test::Command::AssertReturn { line, ref action, ref expected } => {
                ...
                match result {
                    Ok(result) => {
                        ...
                        let actual_result = result.into_iter().collect::<Vec<sophon_wasm::RuntimeValue>>();
            ...
            &test::Command::AssertReturnCanonicalNan { line, ref action } | &test::Command::AssertReturnArithmeticNan { line, ref action } => {
                ...
                match result {
                    Ok(result) => {
                        for actual_result in result.into_iter().collect::<Vec<sophon_wasm::RuntimeValue>>() {
                            match actual_result {
